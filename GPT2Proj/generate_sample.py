...
def predict_one_sample(model:GPT2LMHeadModel, tokenizer, device, args, content):
    ...
    # 将input_ids进行扩充，扩充到需要预测摘要的个数，即batch_size
        ...
        for index in range(args.batch_size):
            ...
            for token_index in range(len(generated)):
                ...
                if generated[token_index][index] != sep_id:
                    ...
                    break
            # 将token_id序列变成汉字序列，去除"##"，并将[unused11]替换成空格
            candidate_responses.append(
                "".join(tokenizer.convert_ids_to_tokens(responses)).replace("##", "").replace("[unused11]", " "))
